// Earlier rational-quadratic q=2 definition retained for comparison.
// This is the original full-data model with only the fixed RQ shape changed.
      ...
      for (i in 1:N) {
        ...
        if (i < N) {
          for (j in (i + 1):N) {
            ...
            real correlation;
            if (kernel_id == 5) {
              // Rational quadratic with fixed q = 2:
              // [1 + d^2 / (4 rho^2)]^(-2).
              correlation = square(inv(1 + 0.25 * square(r)));
            } else if (kernel_id == 6) {
