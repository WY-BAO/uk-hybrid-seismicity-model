"""Derive the L4/no-selection activity rate above Mw 2.

The existing four-chain no-selection posterior was fitted with Mw 3 as the
reporting threshold.  In this Stan program ``mw_min`` is used only in the
generated-quantities block, so the same posterior can be re-expressed exactly
at Mw 2 without rerunning MCMC.
"""
...
L4_DIR = Path(__file__).resolve().parent
BASE_INPUT = L4_DIR / "output" / "l5_input_regional.json"
BASE_RESULT = (
    L4_DIR
    / "output"
    / "selection_ablation"
    / "selection_ablation_no_selection_result.json"
)
BASE_RUN_DIR = L4_DIR / "output" / "selection_ablation" / "stan_run_no_selection"
OUTPUT_DIR = L4_DIR / "output" / "selection_ablation" / "mw2"
STAN_FILE = L4_DIR / "stafford_l5_no_selection.stan"
STAN_EXE = STAN_FILE.with_suffix(".exe")
CMDSTAN_PATH = Path(cmdstan_path())
L5_MW2_RESULT = L4_DIR / "output" / "mw2" / "l5_result_regional_mw2.json"
TARGET_MW_MIN = 2.0
...
def run_generated_quantities(
    ...
    for chain, fitted_params in enumerate(chain_files, start=1):
        output_file = output_dir / f"l4_mw2_generated_quantities_chain_{chain}.csv"
        command = [
            str(STAN_EXE),
            "generate_quantities",
...
def markdown_summary(result: dict[str, Any]) -> str:
    ...
    comparison = result["comparison_with_full_l5_mw2"]
    diagnostics = result["diagnostics"]
    return f"""# L4/no-selection result at Mw >= 2

The existing no-selection posterior was re-expressed at Mw = 2 using Stan's
standalone generated-quantities step. No new MCMC sampling was required.
...
## Comparison with full L5 at Mw >= 2

- L5 mean lambda: {comparison['full_l5_mean_lambda_mw_ge_2_per_year']:.3f} yr^-1.
- L4/no-selection mean lambda: {lam2['mean']:.3f} yr^-1.
- Change without selection: {comparison['posterior_mean_lambda_percent_change']:.1f}%.
- Change in mean b: {comparison['posterior_mean_b_percent_change']:.1f}%.

## Diagnostics and validation

- Posterior draws: {result['posterior_draws']} from {result['chains']} chains.
- Divergent transitions: {diagnostics['divergences']}.
- Maximum-treedepth hits: {diagnostics['treedepth_hits']}.
...
- Stan versus independent Mw-2 calculation, maximum absolute difference:
  {result['validation']['max_abs_difference_stan_vs_manual_mw2']:.3e}.
"""
...
def main() -> int:
    ...
    chain_files = sorted(BASE_RUN_DIR.glob("stafford_l5_no_selection-*.csv"))
    if len(chain_files) != 4:
        raise RuntimeError(f"Expected four chain CSVs, found {len(chain_files)}")
    ...
    mw2_input_file = OUTPUT_DIR / "l4_input_regional_mw2.json"
    ...
    for chain, (original, gq) in enumerate(zip(original_frames, gq_frames), start=1):
        ...
        if not {"b", "lambda_mw_min"}.issubset(gq.columns):
            raise RuntimeError(f"Generated chain {chain} lacks required quantities")
    ...
    draws_file = OUTPUT_DIR / "l4_posterior_draws_mw2.csv"
    draws.to_csv(draws_file, index=False, float_format="%.12g")

    stansummary_file = OUTPUT_DIR / "stansummary_l4_mw2.csv"
    ...
    l4_lam2 = quantile_summary(lambda2_stan)
    l4_b = quantile_summary(b)

    l5_result = json.loads(L5_MW2_RESULT.read_text(encoding="utf-8"))
    l5_lam2_mean = float(
        l5_result["posterior"]["lambda_mw_ge_2_per_year"]["mean"]
    )
    l5_b_mean = float(l5_result["posterior"]["b"]["mean"])
    comparison = {
        "difference_definition": "L4/no-selection minus full L5 with selection",
        "full_l5_mean_lambda_mw_ge_2_per_year": l5_lam2_mean,
        "l4_no_selection_mean_lambda_mw_ge_2_per_year": l4_lam2["mean"],
        "posterior_mean_lambda_percent_change": 100.0
        * (l4_lam2["mean"] / l5_lam2_mean - 1.0),
        "posterior_mean_b_percent_change": 100.0 * (l4_b["mean"] / l5_b_mean - 1.0),
    }

    original_diagnostics = base_result["result"]["diagnostics"]
    result = {
        "case": "L4/no-selection controlled ablation",
        ...
        "diagnostics": {
            ...
            "original_fit_max_rhat": float(original_diagnostics["max_rhat"]),
            "original_fit_min_ess_bulk": float(original_diagnostics["min_ess_bulk"]),
        },
        "validation": {
    ...
    result_file = OUTPUT_DIR / "l4_result_regional_mw2.json"
    result_file.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    summary_file = OUTPUT_DIR / "RESULTS_L4_MW2.md"
